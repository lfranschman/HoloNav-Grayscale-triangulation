...
from calibration_helpers import get_mat_c_to_w_series, get_mat_w_to_o, get_mat_m_to_o_series, \
    get_lut_pixel_image, get_mat_divots_filename
...
def find_optical_spheres():
    ...
    # if True:
    if False:
        timestamp = data.acquisitions["qr_code_position"].index[
            config.qr_code_optical_calibration_starting_time]
    ...
    for camera in ["vl_front_left_cam", "vl_front_right_cam"]:
        ...
        for frame_id in range(87, 892):
            ...
            # if True:
            if False:
                qr_code_index = data.acquisitions["qr_code_position"].index.get_loc(timestamp,
                                                                                    method='nearest')
            ...
            for i, sphere_pos in enumerate(sphere_positions):
                ...
                # t1 = time.time()
                coord = get_lut_pixel_image(lut_projection, pos_sphere1_c[0], pos_sphere1_c[1],
                                            pos_sphere1_c[2])
